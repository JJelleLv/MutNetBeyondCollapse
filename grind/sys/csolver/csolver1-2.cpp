...
double TExternvar::valueat (double t)
{
   ...
   if (data.ncols == 2)
   {
      ...
      previ = it;
      return  data (it - 1, 1) + (data (it, 1)  - data (it - 1, 1)) / (nextt - prevt) * (t - prevt);;
   }
   else
   {
      int it = floor (t);
      if (it <= data.nrows && it >= 0)
...
double TModel::rednoise (double t, double T0, double lambda, double beta, int iset /*= 1*/, double deltat /*= 1*/)
{
   ...
   if (rednoisesets[iset].data.numel == 0)
   {
      if (deltat == 1)
      {
         ...
         rednoisesets[iset].data.setsize (round (tend) + 1, 1);
         rednoisesets[iset].data (0) = T0;
         for (int i = 1; i <= tend; i++)
            rednoisesets[iset].data (i) = (1 - 1 / lambda) * (rednoisesets[iset].data (i - 1) - T0) + T0 + beta * randn();
      }
      else
      {
         rednoisesets[iset].tofloor = true;
...
//   %t is the time of the first point
//%  simple Euler
   tnew = min(nextt,t + forwrd_h); //%adapt the time step if approaching an output point
   forwrd_h=abs(tnew-t);
   ynew = y + f .*  h;
   if anyNonNegative
       ndx = nonNegative( ynew(nonNegative) < 0 );
