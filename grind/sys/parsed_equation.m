classdef parsed_equation < handle
    ...
    methods
        ...
        function value = getstructure1(obj)
        ...
        function subeqs=factorout(obj,oper)
            %split an equation in subequations with the same or smaller precedence as
            %oper (default oper +)
            %s1=factorout(parsed_equation('(a+b)*(c+d)+s*sin(x)'))
            % returns:
            %(a+b)*(c+d)
            %s*sin(x)
            %
            function [rec1,rec2,recs]=spliteq(recs,rec,le_opers)
                if isstruct(rec)
                    if any(strcmp(rec(end).oper,le_opers))
                        f1=find(strcmp({rec.leftvar},rec(end).args{1}));
                        if isempty(f1)
                            recs{end+1}=rec(end).args{1};
                            rec1=[];
                        else
                            [rec1,~,recs]=spliteq(recs,rec(1:f1),le_opers);
                        end
                        
                        f2=find(strcmp({rec.leftvar},rec(end).args{2}));
                        if isempty(f2)
                            recs{end+1}=rec(end).args{2};
                            rec2=[];
                        else
                            [rec1,rec2,recs]=spliteq(recs,rec(1:f2),le_opers);
                        end
            ...
            s1=parsed_equation.operinfo;
            prec= s1.prec(strcmp(s1.opers,oper));
            le_opers=s1.opers(s1.prec>=prec);
            if ~isa(obj,'parsed_equation')
                obj=parsed_equation(obj);
            end
            
            rec=obj.structure;
            recs={};
            [~,~,recs]=spliteq(recs,rec,le_opers);
            for i=1:length(recs)
                s1=recs{i};
                if isstruct(s1)
                    s1(end).leftvar='ans';
        ...
        function obj1 = runmupad(obj,comm)
            ...
            if ~isvectorized
                repstr={'[.][*]','*';'[.][/]','/';'[.][^]','^'};
                s1=regexprep(s1,repstr(:,1),repstr(:,2));               
            end
...
while ~found
    while ((f > 0)&&(f<=length(s)))&&(isvar(f)||isfield(f)||((s{f}(1)>='0')&&(s{f}(1)<='9')))
...
for i=1:length(fss)
    fss{i}=regexprep(fss{i},'[$](?=[\*/\\^''])','.'); %repair each is a cell
end
...
if ~isempty(f1)
    s = fs{f1};
    k = 1;
    while f1+k<=length(fs)&&types(f1+k)==parsed_equation.vr.field
