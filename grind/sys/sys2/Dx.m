%Dx   Single partial derivative to spatial x (=column) direction
%   Defines the gradient in spatial coordinates [dN/dx] to be used for example for advection. Three methods are implemented:
...
% 
%   Usage:
%   Dx(N) defines in a model a partial derivative of the variable N with respect to 
%     the spatial x (=column) coordinates (except for single row variables, then y = column).
%  
%
%   See also model, Dy, Dxx, Dyy, neighborcells, definespace
%
...
if ~isfield(g_grind,'space')||isempty(g_grind.space)
    warning('grind:Dx','No space defined, set to defaults');
    definespace('-d');
end
