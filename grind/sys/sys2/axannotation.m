%axannotation - annotation in an axis
%  Completely compatible with the MATLAB command annotation, but the position
%  of the annotation is in axis units, and instead of the handle to a figure
%  a handle to an axis should be given. The axis units are saved in the
%  userdata.
%
% See also annotation
function hout=axannotation(varargin)
if nargin<2
    error('GRIND:axannotation:ArgError','Not enough arguments')
end
...
if (length(varargin)>1)  && isnumeric(varargin{1}) && (length(varargin{1})==2) && isnumeric(varargin{2})
    [figx,figy]= dsxy2figxy(hAx,varargin{1},varargin{2});  
    hout=annotation(hfig,type,figx,figy,varargin{3:end});
    ud.x=varargin{1};
    ud.y=varargin{2};
    set(hout,'userdata',ud);
elseif (~isempty(varargin))  && isnumeric(varargin{1}) && (length(varargin{1})==4)
    [figpos]= dsxy2figxy(hAx,varargin{1});  
    hout=annotation(hfig,type,figpos,varargin{2:end}); 
    ud.pos=varargin{1};
    set(hout,'userdata',ud);
else
   error('GRIND:axannotation:ArgError','Error parsing arguments');
end
